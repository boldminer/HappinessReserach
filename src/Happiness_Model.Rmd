...
This dataset measures the happiness rank and score among 155 countries globally based on a several factors such as,Economy, generosity, life expectancy, family, trust in government,  and freedom. 
...
```
This correlation analysis shows the GDP.per.capita is the most significant, Healthy.life.expectancy to make life choices is the second.
...
```
This linear model suggests GDP,SicialSUpport, freedom and LifeExpectancy is significant with a overall Adjusted R-squared:  0.7703 
MSE is 0.27187
...
```
P-value is near zero Reject H0 Reg1, the one with all factors is better
...
#### Prune the tree to get rid off some branches :
...
#### Finding the best pruned tree using cross-validation on the training set:
...
```
we notice the accuracy reach the best level when there is 5 branches. so Let's prune the tree! 
```{r}
...
The tree method has a lower error rate.( 0.0708% than 9.61) It performs better than the logistic regression one. 
